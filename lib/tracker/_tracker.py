...
class Tracker(object):
    ...
    @staticmethod
    def crop_patch_fast(im_color, im_ir, box, scale_factor, out_size):  # [x, y, x, y]
        pos = (box[:2] + box[2:]) / 2
        wh = box[2:] - box[:2] + 1
        ...
        # a = out_size / (box[2] - box[0])
        # b = out_size / (box[3] - box[1])
        # c = -a * box[0]
        # d = -b * box[1]
        ...
        #     cv2.rectangle(im_show, (int(out_box[0]), int(out_box[1])),
        #                   (int(out_box[2]), int(out_box[3])), (0, 255, 0),
        #                   3)
        #     cv2.imshow('search_outbox', im_show)
        #     cv2.waitKey()

        return patch_color, patch_ir, out_box, [a, b, c, d]
