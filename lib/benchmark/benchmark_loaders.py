...
def load_GTOT(root):
    video_name_list = np.loadtxt(os.path.join(root, 'list.txt'), dtype=str)

    name = 'GTOT'

    video_list = []
    for v_n in tqdm(video_name_list, ascii=True, desc='loading %s' % name):
        gt_path = os.path.join(root, v_n, 'groundTruth_v.txt')
        color_im_dir = os.path.join(root, v_n, 'v')
        ir_im_dir = os.path.join(root, v_n, 'i')

        color_im_list: strList = os.listdir(color_im_dir)
        color_im_list = sorted(color_im_list)
        ir_im_list: strList = os.listdir(ir_im_dir)
        ir_im_list = sorted(ir_im_list)

        gts = np.loadtxt(gt_path, delimiter=' ').tolist()  # [x y w h]
        if v_n == 'Otcbvs':
            color_ims = [os.path.join(color_im_dir, im_f) for im_f in color_im_list if 'bmp' in im_f]
            ir_ims = [os.path.join(ir_im_dir, im_f) for im_f in ir_im_list if 'bmp' in im_f]
        else:
            color_ims = [os.path.join(color_im_dir, im_f) for im_f in color_im_list if 'png' in im_f]
            ir_ims = [os.path.join(ir_im_dir, im_f) for im_f in ir_im_list if 'png' in im_f]

        video_list.append([v_n, color_ims, ir_ims, gts, ''])

    return video_list

def load_RGBT234(root):
    video_name_list = np.loadtxt(os.path.join(root, 'list.txt'), dtype=str)
    ...
    video_list = []
    for v_n in tqdm(video_name_list, ascii=True, desc='loading %s' % name):
        gt_path = os.path.join(root, v_n, 'visible.txt')
        color_im_dir = os.path.join(root, v_n, 'visible')
        ir_im_dir = os.path.join(root, v_n, 'infrared')
        ...
        color_ims = [os.path.join(color_im_dir, im_f) for im_f in color_im_list if 'jpg' in im_f]
        ir_ims = [os.path.join(ir_im_dir, im_f) for im_f in ir_im_list if 'jpg' in im_f]

        video_list.append([v_n, color_ims, ir_ims, gts, ''])

    return video_list

def load_LasHeR_test(root):
    video_name_list = np.loadtxt(os.path.join(root, 'list.txt'), dtype=str)
    ...
    video_list = []
    for v_n in tqdm(video_name_list, ascii=True, desc='loading %s' % name):
        gt_path = os.path.join(root, v_n, 'init.txt')
        color_im_dir = os.path.join(root, v_n, 'visible')
        ir_im_dir = os.path.join(root, v_n, 'infrared')
...
def load_VTUAV(root):
    video_name_list = np.loadtxt(os.path.join(root, 'VTUAV-ST.txt'), dtype=str)

    name = 'VTUAV'

    video_list = []
    for v_n in tqdm(video_name_list, ascii=True, desc='loading %s' % name):
        gt_path = os.path.join(root, v_n, 'rgb.txt')
        color_im_dir = os.path.join(root, v_n, 'rgb')
        ir_im_dir = os.path.join(root, v_n, 'ir')
