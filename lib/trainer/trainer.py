...
from lib.trainer.interpolate_embeddings import interpolate_embeddings
...
class Trainer(object):
    ...
    def init(self, **kwargs):
        ...
        self.cfg.trainer.current_epoch = self.cfg.trainer.start_epoch

        # build optimizer and lr_scheduler
        self.optimizer = self.build_optimizer(self.cfg.trainer.current_epoch)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i_epoch in range(self.cfg.trainer.start_epoch, self.cfg.trainer.end_epoch):
            ...
            self.writer_train.add_scalar(f'Metric/lr', self.lr_scheduler.get_last_lr()[-1], i_epoch + 1)
            ...
            if self.cfg.trainer.current_epoch == self.cfg.trainer.UNFIX_EPOCH:
                self.logger.debug('training backbone')
                self.optimizer = self.build_optimizer(self.cfg.trainer.current_epoch)
            if self.cfg.trainer.current_epoch == self.cfg.trainer.UNFIX_EPOCH2:
                self.logger.debug('training backbone')
                self.optimizer = self.build_optimizer(self.cfg.trainer.current_epoch)
                # self.lr_scheduler = self.build_lr_scheduler()
            # if self.cfg.trainer.stage == 'stage2':
            #     # change learning rate
            #     print(f"Before step: {self.optimizer.param_groups[0]['lr']}")
            #     self.lr_scheduler.step()
            #     print(f"After step: {self.optimizer.param_groups[0]['lr']}")
            ...
            # validate
                ...
                #with torch.no_grad():
            #    self.validate()
            if self.cfg.trainer.stage == 'stage1':
                self.lr_scheduler.step()
            elif self.cfg.trainer.stage == 'stage2':# and self.cfg.trainer.current_epoch > self.cfg.trainer.UNFIX_EPOCH2 + 13:
                # change learning rate
                print(f"Before step: {self.optimizer.param_groups[0]['lr']}")
                self.lr_scheduler.step()
                print(f"After step: {self.optimizer.param_groups[0]['lr']}")
    ...
    def build_model(self, model_builder, find_unused):
        ...
        model = model_builder(self.cfg).cuda()
    ...
    def build_dataloader(self, dataset_fn):
        ...
        # return loader_train, sampler_train

    def build_optimizer(self, current_epoch=1):
        ...
        for n, param in model.backbone.named_parameters():
            # if "pos" in n or "GAT_Branch" in n:
            if "GAT_Branch" in n:
        ...
        if pretrain_name is not None:
            ...
            param_dicts = []
# stage1, not alive in stage2
#             if current_epoch >= self.cfg.trainer.UNFIX_EPOCH:
...
#stage2, fix backbone and head, only train lddiffuse
            if current_epoch >= self.cfg.trainer.UNFIX_EPOCH2:
                for n, param in model.named_parameters():
                    if "lddiffuse" in n or "projector2" in n or "head_diffuse" in n:
                        param.requires_grad = True
                    else:
                        param.requires_grad = False
        ...
        # param_list = param_list + [n for n, p in model.backbone.named_parameters() if p.requires_grad]
    ...
    def load_pretrain(self):
        if self.cfg.trainer.pretrain is not None:
            ...
            if isinstance(checkpoint, dict) and 'model' in checkpoint:
                processed_dict= checkpoint['model']
                model_dict = self.model_without_ddp.state_dict()

                pretrained_dict = dict()
                ori_keys = list(processed_dict.keys())
                for ori_key in ori_keys:
                    if "backbone.model" in ori_key:
                        revised_key = ori_key.replace("backbone.model", "backbone")
                        pretrained_dict[revised_key] = processed_dict[ori_key]
                    else:
                        pretrained_dict[ori_key] = processed_dict[ori_key]

                # pretrained_dict_rgbir = dict()
                # ori_keys = list(pretrained_dict.keys())
                # for ori_key in ori_keys:
                #     if "backbone.blocks" in ori_key:
                #         revised_key = ori_key.replace("backbone.blocks", "backbone.blocks_p")
                #         pretrained_dict_rgbir[revised_key] = pretrained_dict[ori_key]
                #         pretrained_dict_rgbir[ori_key] = pretrained_dict[ori_key]
                #     else:
                #         pretrained_dict_rgbir[ori_key] = pretrained_dict[ori_key]
                ...
                model_dict = interpolate_embeddings(256, 16, model_dict, is_pos_embed=True)
                model_dict = interpolate_embeddings(128, 16, model_dict, is_pos_embed=False)
    ...
    def resume(self):
        if self.cfg.trainer.resume is not None:
            ...
            #     if "backbone.blocks" in ori_key:
            #         revised_key = ori_key.replace("backbone.blocks", "backbone.blocks_p")
            #         pretrained_dict_rgbir[revised_key] = processed_dict[ori_key]
            #         pretrained_dict_rgbir[ori_key] = processed_dict[ori_key]
            #     else:
            #         pretrained_dict_rgbir[ori_key] = processed_dict[ori_key]
            #
            # filter_pretrained_dict = {k: v for k, v in pretrained_dict_rgbir.items() if k in model_dict}
            # model_dict.update(filter_pretrained_dict)
            ...
            # build optimizer and lr_scheduler
            # self.optimizer = self.build_optimizer(self.cfg.trainer.start_epoch)
            # self.lr_scheduler = self.build_lr_scheduler()
            ...
            if self.cfg.trainer.start_epoch + 1 != self.cfg.trainer.UNFIX_EPOCH2:
