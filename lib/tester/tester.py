...
class Tester(object):
    ...
    def create_tracker(self):
        ...
        self.model = model_builder(self.exp_cfg).eval()
    ...
    def save_common2(self, video_name, box_list, time_list, score_list):
        res_path = os.path.join(self.tester_cfg.res_dir, 'test_{}.txt'.format(video_name))
        time_path = os.path.join(self.tester_cfg.res_dir, 'times', '{}_time.txt'.format(video_name))
        score_path = os.path.join(self.tester_cfg.res_dir, 'scores', '{}_confidence.txt'.format(video_name))
    ...
    def run_ope(self):  # [x y x y]
        ...
        if self.tester_cfg.num_process == 0:
            for video_idx, (video_name, color_im_list, ir_im_list, gt_list, lang) in enumerate(seqs):
                res_path = os.path.join(self.tester_cfg.res_dir, '{}.txt'.format(video_name))
                # res_path = os.path.join(self.tester_cfg.res_dir, 'test_{}.txt'.format(video_name))
                if os.path.exists(res_path):
                    print('-1')
                    continue
                seq_info = [video_idx, video_name, color_im_list, ir_im_list, gt_list, lang, len(seqs)]
    ...
    def run_seq(self, seq_info, num_gpu):
        video_idx, video_name, im_color_list, im_ir_list, gt_list, lang, seq_num = seq_info
        ...
        if ('lasot' in self.tester_cfg.benchmark or 'tnl2k' in self.tester_cfg.benchmark
                ...
                or 'GTOT' in self.tester_cfg.benchmark or 'RGBT234' in self.tester_cfg.benchmark or 'LasHeR_test' in self.tester_cfg.benchmark
                or 'VTUAV' in self.tester_cfg.benchmark):
        ...
        out_results = self.tracking_loop(im_color_list, im_ir_list, gt_list, lang, video_name)
        ...
        if ('lasot' in self.tester_cfg.benchmark or 'tnl2k' in self.tester_cfg.benchmark
                ...
                or 'trackingnet' in self.tester_cfg.benchmark or 'otb99lang' in self.tester_cfg.benchmark
                or 'trek150' in self.tester_cfg.benchmark or 'itb' in self.tester_cfg.benchmark
                or 'LasHeR_test' in self.tester_cfg.benchmark):
            self.save_common(video_name, box_list, time_list, score_list)

        elif ('GTOT' in self.tester_cfg.benchmark or 'RGBT234' in self.tester_cfg.benchmark or 'VTUAV' in self.tester_cfg.benchmark):
            self.save_common2(video_name, box_list, time_list, score_list)
    ...
    def tracking_loop(self, im_color_list, im_ir_list, gt_list, lang, video_name):
        fps_list = np.zeros([len(im_color_list), 1])
        box_list = np.zeros([len(im_color_list), 4])
        score_list = np.zeros([len(im_color_list), 1])
        time_list = np.zeros([len(im_color_list), 1])
        ...
        description_count = 0
        for im_i in range(0, len(im_color_list)):
            img_color = cv2.imread(im_color_list[im_i])
            img_ir = cv2.imread(im_ir_list[im_i])
            gt = np.array(gt_list[0])  # [x y w h]
            if self.tester_cfg.benchmark == 'GTOT':
                gt[2] = gt[2] - gt[0]
                gt[3] = gt[3] - gt[1]
            ...
            if im_i == 0:
                ...
                self.tracker.init(img_color, img_ir, gt, language=lang_encode)  # [x y w h]
                ...
                predict_box, predict_score, visualization = \
                    self.tracker.track(img_color, img_ir, visualize=self.tracker_cfg.visualize)  # [x y w h]
