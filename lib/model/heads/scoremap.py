...
class ScoreMap(BaseHead):
    ...
    def __init__(self, args, freeze_bn=False):
        ...
        '''center'''
        self.conv1_center = conv(self.in_channels, self.inter_channels, freeze_bn=freeze_bn)
        self.conv2_center = conv(self.inter_channels, self.inter_channels // 2, freeze_bn=freeze_bn)
        self.conv3_center = conv(self.inter_channels // 2, self.inter_channels // 4, freeze_bn=freeze_bn)
        self.conv4_center = conv(self.inter_channels // 4, self.inter_channels // 8, freeze_bn=freeze_bn)
        self.conv5_center = nn.Conv2d(self.inter_channels // 8, 1, kernel_size=1)
    ...
    def forward(self, x, return_dist=True, softmax=True):
        ...
        score_map_tl, score_map_br, score_map_center = self.get_score_map(x)

        score_center_vec = score_map_center.view((-1, self.feat_sz * self.feat_sz))  # (batch, feat_sz * feat_sz)
        prob_vec_center = nn.functional.softmax(score_center_vec, dim=1)
        prob_vec_center = prob_vec_center.view((-1, 1, self.feat_sz, self.feat_sz))
        if return_dist:
            coorx_tl, coory_tl, prob_vec_tl = self.soft_argmax(score_map_tl, return_dist=True, softmax=softmax)
            coorx_br, coory_br, prob_vec_br = self.soft_argmax(score_map_br, return_dist=True, softmax=softmax)
            return torch.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz, prob_vec_tl, prob_vec_br, prob_vec_center
        else:
    ...
    def get_score_map(self, x):
        ...
        x_center1 = self.conv1_center(x)
        x_center2 = self.conv2_center(x_center1)
        x_center3 = self.conv3_center(x_center2)
        x_center4 = self.conv4_center(x_center3)
        score_map_center = self.conv5_center(x_center4)

        return score_map_tl, score_map_br, score_map_center
