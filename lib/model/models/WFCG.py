...
class GraphAttentionLayer(nn.Module):
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        batch_size, N, out_features = Wh.size() # number of nodes
...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, nout, nheads, dropout, alpha, num_nodes):
    ...
    def forward(self, x, A=None):
        if A is None:
            adj = torch.eye(2 * self.num_nodes).unsqueeze(0).repeat(x.shape[0], 1, 1).cuda() #[16,512,512]
        else:
            # adj = self.initialize_adjacency_cosine(x)
            adj = A.cuda()
        x = F.dropout(x, self.dropout, training=self.training)
        x1 = torch.cat([att(x, adj) for att in self.attentions], dim=2)
        x2 = F.dropout(x1, self.dropout, training=self.training)
        x2 = F.elu(self.out_att(x2, adj))
        return x1, x2

    def initialize_adjacency_cosine(self, features, K=5):
        """
        基于余弦相似度初始化邻接矩阵。

        Args:
            features (torch.Tensor): 特征矩阵，形状为 [batch_size, num_tokens, feature_dim]
            K (int): 每个token连接的最近邻数量

        Returns:
            adjacency_matrix (torch.Tensor): 邻接矩阵，形状为 [batch_size, num_tokens, num_tokens]
        """
        # 特征归一化
        normalized_features = F.normalize(features, p=2, dim=2)  # [batch_size, num_tokens, feature_dim]
        ...
        # # 移除自连接
        # eye = torch.eye(features.size(1), device=features.device).unsqueeze(0).repeat(features.size(0), 1, 1)
