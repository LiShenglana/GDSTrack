...
class BaseDataset(Dataset):
    ...
    def check_sample(self, frame_list, video_list, sample_range, min_sz=1):
        # anchor_frame_id = np.random.randint(0, len(frame_list) - sample_range*2, 1)[0]
        ...
        # lp = max(anchor_frame_id, 0)
        # rp = min(anchor_frame_id + sample_range, len(frame_list))

        search1_frame_id = np.random.randint(lp, rp, 1)[0]

        lp = max(search1_frame_id, 0)
        rp = min(search1_frame_id + sample_range, len(frame_list))

        search2_frame_id = np.random.randint(lp, rp, 1)[0]

        f_dict1: dict = frame_list[anchor_frame_id]
        x, y, w1, h1 = f_dict1['bbox']
        f_dict2: dict = frame_list[search1_frame_id]
        x, y, w2, h2 = f_dict2['bbox']
        f_dict3: dict = frame_list[search2_frame_id]
        x, y, w3, h3 = f_dict2['bbox']
        ...
        while np.min([w1, h1, w2, h2]) < min_sz:
            ...
            x, y, w2, h2 = f_dict2['bbox']
            f_dict3: dict = frame_list[search2_frame_id]
            x, y, w3, h3 = f_dict3['bbox']

        return f_dict1, f_dict2, f_dict3
    ...
    def parse_frame_lmdb(f_dict: dict, handles, need_language=False):
        color_key = f_dict['color_key']
        f_color_path = f_dict['color_path']
        ir_key = f_dict['ir_key']
        f_ir_path = f_dict['ir_path']
        dataset_name = f_dict['dataset']

        handle = handles[dataset_name]
        binfile = handle.get(color_key.encode('ascii'))
        if binfile is None:
            print("Illegal data detected. %s %s" % (dataset_name, color_key))
            return None, None
        s = np.frombuffer(binfile, np.uint8)
        if s.size == 0:
            print("Illegal size detected. %s %s" % (dataset_name, color_key))
            return None, None
        color_img = cv2.cvtColor(cv2.imdecode(s, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        # cv2.imshow('color_img', color_img)
        # cv2.waitKey()

        binfile2 = handle.get(ir_key.encode('ascii'))
        if binfile2 is None:
            print("Illegal data detected. %s %s" % (dataset_name, ir_key))
            return None, None
        s2 = np.frombuffer(binfile2, np.uint8)
        if s2.size == 0:
            print("Illegal size detected. %s %s" % (dataset_name, ir_key))
            return None, None
        ir_img = cv2.cvtColor(cv2.imdecode(s2, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        ...
        if bbox is not None:
            ...
            bbox = np.array([x, y, w, h])
            # if w ==0:
            #     print('f_color_path:', f_color_path)
            #     print('box:', bbox[0], bbox[1], bbox[2], bbox[3])

        # print("key:", key)

        if not need_language:
            return color_img, ir_img, bbox  # [x y w h]
        else:
            lang = f_dict.get('language', '')
            return color_img, ir_img, bbox, lang  # [x y w h]
    ...
    def crop_patch_fast(self, im_color, im_ir, box, out_size, padding, scale_factor, jitter_f, mask=None):
        img_sz = im_color.shape
        # pad = 14
        # target_pos = [out_size[0] / 2, out_size[1] / 2]
        # target_pos = [random.randint(pad, out_size[0] - pad), random.randint(pad, out_size[1] - pad)]
        # target_sz = [out_size[0] / 2, out_size[1] / 2]

        #unsupervised training, use random crop
        # box_pos = box_pos
        # crop_sz = [img_sz[0] / 15, img_sz[1] / 15]
        # window_sz = np.array(crop_sz) * (1 + padding)
        # # box = np.array([box_pos[0], box_pos[1], crop_sz[0], crop_sz[1]])
        #
        # box = self.cxy_wh_2_bbox(box_pos, window_sz)
        # box = np.array(box)
        # box[2:] = box[2:] - box[:2] + 1
        _, j_center, j_size = self.box_jitter(box, jitter_f)
        #
        ...
        d = -b * y1
        # if crop_sz == 0:
        #      print('j_size0:', j_size[0])
        #      print('j_size1:', j_size[1])
        #      print('box:', box[0], box[1], box[2], box[3])
        #      print('j_center0:', j_center[0])
        #      print('j_center1:', j_center[1])
        #      print('a:', a)
        #      print('b:', b)

        # a = out_size[0] / (box[2] - box[0])
        # b = out_size[1] / (box[3] - box[1])
        # c = -a * box[0]
        # d = -b * box[1]
        ...
        patch_color = cv2.warpAffine(im_color, mapping,
                               (out_size[1], out_size[0]),
                               borderMode=cv2.BORDER_CONSTANT,
                               borderValue=np.array([103.53, 116.28, 123.675]))  # imagenet BGR

        patch_ir = cv2.warpAffine(im_ir, mapping,
                               (out_size[1], out_size[0]),
                               borderMode=cv2.BORDER_CONSTANT,
                               borderValue=np.array([103.53, 116.28, 123.675]))  # imagenet BGR
        ...
        if mask is None:
            ...
            x1, y1, w, h = box
            out_box = np.array([x1, y1, x1 + w - 1, y1 + h - 1])
            ...
            ori_center = np.array([float(out_size[0] / 2), float(out_size[0] / 2)]) #112 - 1
            out_center = out_box[0:2] + 0.5 * out_box[2:4]
            shift = out_center - ori_center
            # test_outbox = True
            # if test_outbox:
            #     im_show = cv2.cvtColor(patch_color, cv2.COLOR_RGB2BGR)
            #     cv2.rectangle(im_show, (int(out_box[0]), int(out_box[1])), (int(out_box[2]), int(out_box[3])), (0, 255, 0),
            #                   3)
            #     cv2.imshow('crop_search_outbox', im_show)
            #     im_show2 = cv2.cvtColor(im_color, cv2.COLOR_RGB2BGR)
            #     cv2.rectangle(im_show2, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])),
            #                   (0, 255, 0),
            #                   3)
            #     cv2.imshow('search_outbox', im_show2)
            #     cv2.waitKey()

            return patch_color, patch_ir, -np.ones((out_size[1], out_size[0])), out_box, shift
            ...
            return patch_color, patch_ir, patch_mask, out_box

    def cxy_wh_2_bbox(self, cxy, wh):
    ...
    def box_jitter(box, jitter_f):
        scale_jitter_f, center_jitter_f = jitter_f  #[0.5, 3]
        box = np.array(box)
    ...
    @staticmethod
    def vertical_flip(image, bbox=None):  # [x, y, w, h]
        ...
        if bbox is None:
            ...
            bbox = np.array([x1, height - 1 - y2, x2, height - 1 - y1])
            bbox[2:] = bbox[2:] - bbox[:2] + 1
