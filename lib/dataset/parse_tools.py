...
class DatasetLoader(object):
    def __init__(self):
        ...
        self.color_img_list = None
        self.ir_img_list = None
        self.gt_list = None
        self.lang_list = None
        self.color_key_list = None
        self.ir_key_list = None
    ...
    def get_frame_dict(self, idx, lmdb_put_img_fn: Callable = None):
        frame_dict_list = []
        for im_i, _ in enumerate(self.color_img_list):

            frame_dict = dict()
            frame_dict['color_path'] = self.color_img_list[im_i]
            frame_dict['ir_path'] = self.ir_img_list[im_i]
            ...
            if self.imw is None or self.imh is None:
                img = Image.open(os.path.join(self.data_dir, self.color_img_list[im_i]))
            ...
            if self.__getattribute__('color_key_list') is None:
                frame_dict['color_key'] = os.path.join(self.video_name,
                                                 self.color_img_list[im_i].split('/')[-1].split('.')[0])
            else:
                frame_dict['color_key'] = self.color_key_list[im_i]

            if self.__getattribute__('ir_key_list') is None:
                frame_dict['ir_key'] = os.path.join(self.video_name,
                                                 self.ir_img_list[im_i].split('/')[-1].split('.')[0])
            else:
                frame_dict['ir_key'] = self.ir_key_list[im_i]
            ...
            frame_dict['length'] = len(self.color_img_list)
            ...
            if lmdb_put_img_fn is not None:
                lmdb_put_img_fn(img_file=os.path.join(self.data_dir, self.color_img_list[im_i]),
                                key=frame_dict['color_key'])
                lmdb_put_img_fn(img_file=os.path.join(self.data_dir, self.ir_img_list[im_i]),
                                key=frame_dict['ir_key'])
                print('process ... {} / {}, {} / {}, save json, save lmdb'.format(
                    idx, len(self.video_list), frame_dict['color_path'], frame_dict['length']))
            else:
