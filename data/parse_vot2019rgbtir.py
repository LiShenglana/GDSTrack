...
class GOT10kLoader(DatasetLoader):
    def __init__(self, name: str, split: str):
        ...
        if 'train' in split:
            self.root = '/home/cscv/Documents/lsl/dataset/VOT2019rgbtir/train'
        # elif 'val' in split:
        #     self.root = '/data1/Datasets/GOT-10k/GOT-10k/val'
        else:
            self.root = '/home/cscv/Documents/lsl/dataset/VOT2019rgbtir/test'
    ...
    def get_video_info(self, video_name: str):
        ...
        color_video_dir = os.path.join(video_dir, 'color')
        ir_video_dir = os.path.join(video_dir, 'ir')
        ...
        color_img_list = np.array([os.path.join(video_name, 'color', img_file) for img_file in color_img_list if '.jpg' in img_file])
        ...
        ir_img_list = np.array(
            [os.path.join(video_name, 'ir', img_file) for img_file in ir_img_list if '.jpg' in img_file])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir', default='/home/cscv/Documents/lsl/dataset/VOT2019rgbtir/LMDB', type=str)
    parser.add_argument('--only_json', dest='only_json', action='store_true', default=False)
    args = parser.parse_args()

    dataset_name = f'vot2019rgbtir_{args.split}'
